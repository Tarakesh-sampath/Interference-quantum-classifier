...
\begin{abstract}
Quantum machine learning (QML) classifiers for near-term hardware often struggle with the extreme measurement costs of fidelity estimation or the unstable optimization landscapes of variational circuits. We propose a measurement-efficient, interference-based quantum classification framework that avoids parameterized training and probability-based decision rules. Applied to histopathological cancer detection, our method demonstrates practical utility on the PatchCamelyon dataset for lymph node metastasis classification. Inference is performed via a fixed quantum interference circuit, deriving the decision directly from the sign of the real part of the linear state overlap. This approach enables inference with a minimal number of measurement shots, potentially as low as a single ancilla measurement in ideal conditions, thereby reducing the measurement overhead compared to fidelity-based techniques. By decoupling learning from quantum execution, static class-representative quantum states are constructed classically. We formalize this linear interference decision primitive and demonstrate its geometric properties as a Hilbert space hyperplane classifier. Extensive simulations validate stable performance under realistic noise parameters, establishing linear quantum interference as a reliable and effective decision primitive for near-term medical imaging applications.
\end{abstract}
...
The PatchCamelyon (PCam) dataset is a widely recognized benchmark for histopathological cancer detection, specifically designed for lymph node metastasis identification \cite{pcam_01,pcam_03}. It consists of 96×96 RGB image patches extracted from whole-slide lymph node histology scans. The dataset is curated for binary classification, distinguishing tumor (metastatic) tissue from non-tumor (benign) tissue, with a balanced distribution of classes to ensure unbiased training and evaluation.
...
The dataset contains over 300,000 patches, providing a rich source for training machine learning models. Due to the high dimensionality and complexity of the images, hybrid quantum-classical approaches like ours leverage classical feature extraction followed by quantum classification to balance computational efficiency and accuracy \cite{pcam_01,pcam_03}.

Figure~\ref{fig:flowchart} illustrates the overall inference pipeline, where classical input images are first encoded into quantum states, followed by fixed interference-based quantum classification. This hybrid approach is motivated by the need to exploit quantum interference advantages while managing the practical constraints of current quantum hardware.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\linewidth]{flowchart_fig1.png}
...
\begin{itemize}
    ...
    \item Using a thorough \textbf{Shot-Complexity Analysis}, we demonstrate that for most classification tasks, accurate inference can be accomplished with minimal measurement shots, potentially as low as single-shot in ideal scenarios.
...
In this work, classical feature extraction is performed using a convolutional neural network (CNN) backbone designed to process the 96×96 RGB histopathology patches from the PCam dataset. The CNN extracts hierarchical features that capture relevant morphological patterns indicative of tumor presence.

Following the CNN backbone, a dimensionality reduction "neck" module compresses the high-dimensional feature maps into a lower-dimensional embedding suitable for quantum encoding. This neck typically consists of a global average pooling layer followed by a fully connected projection layer. The output embedding dimension $d$ is chosen to balance expressivity and quantum resource constraints, directly determining the number of qubits $n = \lceil \log_2 d \rceil$ required for the quantum classifier.

Explicit tensor shapes are as follows:

\begin{itemize}
    \item Input image tensor: $(96, 96, 3)$
    \item CNN feature maps (after final convolutional block): $(h, w, c)$, where $h, w$ depend on CNN architecture
    \item After global average pooling: $(c)$
    \item Fully connected projection output (neck embedding): $(d)$
    \item Quantum feature vector: $(d)$ mapped to $n$ qubits
\end{itemize}

This structured compression ensures that the quantum classifier operates on a manageable number of qubits while retaining discriminative features extracted from the raw image data. The classical-quantum interface is thus clearly defined, facilitating efficient hybrid training and inference.

The CNN backbone and neck are trained end-to-end with the quantum classifier in a hybrid fashion, using classical optimizers and loss functions appropriate for binary classification. This approach leverages the representational power of CNNs while exploiting quantum interference for classification decisions \cite{pcam_01,pcam_03}.
...
We adopt \textbf{amplitude encoding} as the primary feature map $\Phi: \mathbb{R}^d \rightarrow \mathbb{C}^{2^n}$, where a normalized classical vector $\mathbf{x} \in \mathbb{R}^d$ is mapped to the amplitudes of an $n$-qubit quantum state:

\begin{equation}
|\psi\rangle = \sum_{i=0}^{2^n - 1} x_i |i\rangle,
\end{equation}

with $n = \lceil \log_2 d \rceil$. Amplitude encoding provides the highest information density, encoding $d$ features into $n$ qubits, but requires complex state preparation circuits.

Alternative encoding strategies such as angle encoding (using single-qubit rotations $R_y$ or $R_z$) and basis encoding are less information-dense but more hardware-friendly. However, amplitude encoding aligns naturally with the IQC's linear interference primitive, as the real part of the inner product in Hilbert space corresponds directly to the Euclidean dot product in input space.
...
This explicit quantum encoding and circuit design enables the IQC to achieve constant-shot inference with minimal circuit overhead, leveraging the linear interference decision primitive for efficient and noise-resilient classification.

% Continue with next sections...

\section{Training and Hybrid Optimization}

The training process in the Interference-Based Quantum Classifier (IQC) framework primarily focuses on optimizing the classical components of the hybrid system. Specifically, the convolutional neural network (CNN) backbone and the dimensionality reduction neck are trained using classical optimization algorithms such as Adam or stochastic gradient descent (SGD).

The quantum interference circuit used for inference is fixed and non-parameterized, eliminating the need for quantum circuit parameter updates or quantum gradient evaluations. This decoupling simplifies the training loop and avoids common challenges in variational quantum algorithms, such as barren plateaus and noisy gradient estimates.

During training, classical input images are passed through the CNN and neck to produce feature embeddings, which are then encoded into quantum states for inference. The measurement outcomes from the quantum interference circuit provide classification scores used to compute a classical loss function (e.g., cross-entropy). Gradients are backpropagated through the classical network components only, while the quantum measurement acts as a fixed, differentiable layer.

This hybrid training approach leverages the representational power of classical deep learning while exploiting the quantum interference mechanism for efficient and noise-resilient classification decisions. It also reduces quantum hardware demands by minimizing quantum circuit complexity and measurement overhead.

The training loop can be summarized as follows:

\begin{enumerate}
    \item Forward pass: Input images $\rightarrow$ CNN + neck $\rightarrow$ quantum encoding $\rightarrow$ interference circuit measurement $\rightarrow$ classification score.
    \item Loss computation: Calculate classical loss based on measurement outcomes and true labels.
    \item Backpropagation: Compute gradients and update classical network parameters.
    \item Repeat until convergence.
\end{enumerate}

This approach ensures efficient training compatible with current NISQ hardware constraints and leverages the strengths of both classical and quantum computing paradigms.
...
\caption{Architecture diagram of the hybrid classical-quantum classification pipeline. The system integrates a classical CNN backbone, dimensionality reduction neck, quantum state encoding, and a fixed interference-based quantum classifier for efficient and robust inference.}
...
This confirms that \textbf{a minimal number of measurement shots, potentially as low as a single shot,} is sufficient for robust inference, provided the states are sufficiently separated in Hilbert space.
...
\begin{itemize}
    ...
    \item \textbf{Measurement}: minimal number of shots, potentially as low as a single shot, for sign estimation.
