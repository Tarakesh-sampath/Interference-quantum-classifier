...
\begin{abstract}
Quantum machine learning (QML) classifiers proposed for near-term quantum hardware typically rely on variational quantum circuits (VQCs) or fidelity-based similarity estimation. While these approaches offer high expressivity, they are often hindered by extreme measurement costs, unstable optimization landscapes (e.g., barren plateaus), and significant sensitivity to hardware noise, which collectively limit their practical utility on noisy intermediate-scale quantum (NISQ) devices. This work presents a measurement-efficient, interference-based quantum classification framework that moves away from parameterized circuit training and probability-based decision rules. The proposed approach performs inference using a fixed (non-trainable) quantum interference circuit, where the classification decision is derived directly from the real part of the linear overlap between quantum state representations---a quantity accessed through a physical interference signal. By decoupling learning from quantum execution, we enable classical computation to handle the construction of static class-representative quantum states in Hilbert space. We provide a rigorous mathematical formalization of this linear interference decision primitive, investigate its geometric properties as a hyperplane classifier in Hilbert space, and analyze its shot complexity, showing it requires significantly fewer measurements than fidelity-based methods. Through extensive simulation under realistic noise parameters, we demonstrate that the proposed classifier achieves stable performance. Inference reduces to a sign test on a single ancilla observable, yielding constant-shot decision complexity with respect to system size for fixed confidence and non-vanishing margin. This work establishes linear quantum interference as a robust and efficient decision primitive for the next generation of near-term quantum classification systems.
\end{abstract}

\begin{IEEEkeywords}
Quantum Machine Learning, Quantum Classification, Quantum Interference, Linear Overlap, NISQ Devices, Hybrid Quantum--Classical Systems
\end{IEEEkeywords}

\section{Introduction}

The rapid advancement of quantum hardware has spurred intense interest in quantum machine learning (QML), a field that seeks to leverage quantum mechanical phenomena—such as superposition, entanglement, and interference—to enhance data processing and pattern recognition tasks \cite{wang2024review,cerezo2021variational}. Among the various branches of QML, supervised classification has emerged as a primary focus, with researchers proposing diverse architectures including variational quantum classifiers (VQCs), quantum kernel methods, and fidelity-based similarity models \cite{benedetti2019parameterized,abbas2021power,schuld2019feature}. These methods are largely tailored for the Noisy Intermediate-Scale Quantum (NISQ) era, where limited qubit counts and high gate error rates necessitate hardware-efficient designs.

Despite the proliferation of these models, several fundamental bottlenecks impede their transition from theoretical constructs to practical industrial tools. Variational approaches, for instance, rely on an iterative feedback loop between a quantum processor and a classical optimizer. In this "hybrid" loop, the quantum device evaluates a cost function, and the classical optimizer adjusts the parameters of a quantum circuit accordingly. However, the optimization landscape of VQCs is frequently plagued by barren plateaus—regions where gradients vanish exponentially with the number of qubits—making the training process computationally expensive and often unreliable \cite{mcclean2018barren}.
...
This work proposes a paradigm shift in near-term quantum classification. Rather than optimizing parameterized circuits or estimating quadratic probabilities, we ground the classification decision in a linear interference primitive: the real part of the quantum state overlap, $\mathrm{Re}\langle \phi | \psi \rangle$. The proposed framework, termed the Interference-Based Quantum Classifier (IQC), employs a fixed, non-trainable quantum interference circuit for inference. The learning phase---constructing the representative ``memory'' states for each class---is performed entirely through classical preprocessing of the training data. This strict decoupling of learning and execution ensures that the quantum hardware is used only for the task it is uniquely suited for: performing high-dimensional interference in Hilbert space.

Crucially, the IQC framework offers a path toward constant measurement-shot complexity. Because the decision rule depends only on the sign of the interference signal rather than its precise magnitude, the number of measurements required to classify a sample remains nearly constant, regardless of the precision required for the underlying overlap value. This makes the system inherently more robust to the shot noise and decoherence characteristic of current quantum hardware.

The primary contributions of this work are summarized as follows:
\begin{itemize}
    \item We introduce \textbf{linear quantum interference} as a decision primitive for classification, demonstrating its conceptual advantages over quadratic observables like fidelity.
    \item We formalize the \textbf{Decision Geometry} of the IQC, showing that it naturally implements a hyperplane classifier within the exponentially large Hilbert space of the quantum system.
    \item We describe a hardware-efficient \textbf{Interference Circuit} based on the Hadamard test that directly extracts the required decision signal with minimal gate overhead.
    \item We provide a comprehensive \textbf{Shot-Complexity Analysis}, proving that accurate inference can be achieved with $O(1)$ measurement shots for the majority of classification tasks.
    \item We conduct \textbf{Simulated Experiments} under various noise regimes, validating the robustness and efficiency of the proposed method against standard VQC and kernel baselines.
\end{itemize}
...
The landscape of quantum machine learning (QML) has evolved rapidly, moving from early theoretical subroutines for linear algebra to hardware-efficient architectures designed for NISQ hardware. This section categorizes the most relevant approaches and highlights the research gaps addressed by the linear interference-based framework.

\subsection{Variational Quantum Classifiers (VQCs)}

Variational quantum classifiers represent the most prominent family of NISQ-era machine learning models \cite{benedetti2019parameterized,abbas2021power}. VQCs use a parameterized quantum circuit (PQC) as an ansatz, whose parameters are optimized to minimize a classical cost function. While highly flexible, VQCs are notoriously difficult to train. The existence of barren plateaus—regions in the parameter space where gradients vanish exponentially with the number of qubits—often renders gradient-based optimization infeasible for larger systems \cite{mcclean2018barren}. Furthermore, VQCs require a high volume of measurement shots at each optimization step to achieve low-variance gradient estimates, placing a heavy burden on the quantum processor's duty cycle.

\subsection{Quantum Kernel and Similarity-Based Methods}

Quantum kernel methods avoid the optimization difficulties of VQCs by mapping data into high-dimensional Hilbert space and computing similarities between pairs of data points \cite{schuld2019feature,havlicek2019supervised}. These similarities, typically defined as the fidelity $F(\rho, \sigma) = \text{Tr}(\sqrt{\sqrt{\rho}\sigma\sqrt{\rho}})^2$, are used as kernel entries in classical algorithms like Support Vector Machines (SVMs). While mathematically elegant, kernel methods suffer from quadratic scaling in terms of quantum circuit executions: to classify a new point against a dataset of size $N$, one must perform $O(N)$ similarity measurements. Moreover, fidelity is a quadratic observable that removes relative phase information, which can be detrimental in cases where the decision boundary depends on the underlying sign structure of the state amplitudes.

\subsection{Quantum State Discrimination and Metric Learning}

Quantum state discrimination (QSD) approaches treat classification as the task of identifying which of several known quantum states a given system is in \cite{sergioli2019binary,gonzalez2021classification}. This perspective shifts the focus from circuit parameters to the geometric arrangement of states in Hilbert space. Relatedly, quantum metric learning attempts to learn a feature map that maximizes the distance between states representing different classes while minimizing it for states of the same class. Our work builds on these geometric foundations but introduces a distinct inference mechanism: we use a fixed interference circuit to extract a linear overlap signal, providing a simpler and more measurement-efficient alternative to full state tomography or fidelity estimation.

\subsection{Interference-Based QML}

The use of interference for pattern recognition was pioneered in works such as the distance-based classifier by Schuld et al. \cite{schuld2017distance,schuld2018book}, which utilized the Hadamard test to compute distances between vectors. However, many early implementations required the use of QRAM or complex state-preparation routines that are difficult to realize on NISQ devices. In contrast, our framework focuses on purely linear interference as a decision primitive using static, pre-computed class centroids, significantly reducing both the circuit depth and the sampling overhead required for practical inference.
...
The core theoretical contribution of this work is the use of linear interference as the primary signal for quantum classification. This section formalizes the distinction between linear and quadratic similarity measures and analyzes the resulting decision geometry in Hilbert space.

\subsection{Linear vs. Quadratic Similarity}

In traditional quantum classification frameworks, similarity between states is often defined via the fidelity $F(\phi, \psi) = |\langle \phi | \psi \rangle|^2$. While fidelity is a natural metric for state closeness, its quadratic nature introduces several practical disadvantages for inference on NISQ devices:
\begin{enumerate}
    \item \textbf{Phase Erasure}: The absolute square operation $|\cdot|^2$ removes the relative phase and sign information between state amplitudes. In many machine learning contexts, the sign of the overlap (interference) carries critical directional information that is lost in fidelity estimation.
...
\end{equation}
This summation is identical to the Euclidean inner product in $\mathbb{R}^{2^{n+1}}$. The decision boundary $\text{Re}\langle \phi | \psi \rangle = 0$ is therefore the locus of all states in $\mathcal{H}$ that are "mutually orthogonal" under this real-part inner product. 
...
\end{equation}
Crucially, since $(1-p)^L > 0$, the **sign** of the expectation value is preserved. The classification remains correct as long as the shot noise does not exceed the attenuated signal.
...
\end{equation}
If the dephasing is symmetric around zero, the expected decision score remains unbiased. However, systematic phase drift can shift the decision boundary. This suggests that the IQC is more tolerant to stochastic noise (like depolarization) than to systematic calibration errors.

\section{Practical Considerations for Hardware Deployment}

Transitioning the IQC from simulation to real quantum hardware requires careful consideration of gate decomposition and ancilla management.

\subsection{Gate Synthesis and Transpilation}

The Hadamard test requires controlled-unitary operations $CU$. On most NISQ platforms, such as those based on transmon or trapped-ion qubits, the native gate set is limited to single-qubit rotations and a two-qubit entanglement gate (e.g., CNOT or Mølmer-Sørensen). The decomposition of a general $CU$ into native gates can significantly increase the circuit depth $L$, potentially exacerbating the noise effects analyzed in Section V. For the IQC to remain measurement-efficient, it is imperative to use hardware-efficient state preparation unitaries $V(\theta)$ that decompose into short-depth sequences.
...
\begin{itemize}
    \item \textbf{Space Complexity}: To represent a $d$-dimensional feature vector, the IQC requires $n = \lceil \log_2 d \rceil$ qubits, achieving an exponential compression in state representation. Classically, the centroid states require $O(d)$ complex double-precision floats.
...
The reduction in measurement shots has a direct impact on the energy consumption of the quantum-classical system. In superconducting architectures, each measurement shot involves microwave pulses and high-speed classical digitization, consuming significant power at the control electronics layer. By reducing the number of shots from $10^4$ (typical for fidelity estimation) to $10^1-10^2$ (for IQC sign estimation), we achieve a reduction in the "energy per inference" metric. This positions interference-based QML as a more sustainable alternative for large-scale data processing in the future quantum ecosystem.
...
The learning objective is to find a normalized quantum state $|\phi^{(c)}\rangle$ that best represents the distribution of training samples belonging to class $c \in \{-1, +1\}$. In this work, we adopt a \textbf{centroid-based aggregation rule}, which is conceptually equivalent to calculating the mean vector in Hilbert space.
...
This static framework serves as the foundation for the Interference-Based Quantum Classifier. Future work will explore how these static representatives can be dynamically adapted (multi-state ensembles or adaptive memory), but the core inference mechanism remains the linear overlap presented here.

\section{Experimental Evaluation}

We evaluate the performance of the Interference-Based Quantum Classifier through high-fidelity numerical simulation, focusing on accuracy, measurement efficiency, and robustness to noise.

\subsection{Simulated Environment and Datasets}

The simulations are conducted using a statevector-based quantum simulator, with a custom noise model incorporated to emulate NISQ hardware characteristics. All experiments are performed using classical statevector simulation; the reported shot complexity refers to the physical measurement process rather than the simulation runtime.
\begin{itemize}
    \item \textbf{Data Embedding}: Input features are mapped to quantum states using amplitude encoding. For $d$-dimensional data, we use $n = \lceil \log_2 d \rceil$ qubits.
    \item \textbf{Noise Model}: We implement a depolarizing noise channel with error rate $p \in [0.01, 0.1]$ and thermal relaxation times ($T_1, T_2$) consistent with current superconducting processors.
    \item \textbf{Benchmarks}: We compare our IQC against a standard Variational Quantum Classifier (VQC) with a parameterized hardware-efficient ansatz and a Fidelity-Based similarity classifier.
\end{itemize}
...
\begin{table}[ht]
\centering
\caption{Inference Accuracy vs. Measurement Shots}
...
The experimental results and theoretical analysis presented in the preceding sections highlight linear quantum interference as a potent and efficient alternative to the prevailing quadratic similarity paradigm in NISQ-era machine learning.

\subsection{Implications for Measurement-Free Quantum Computing}

The realization that classification can be performed with $O(1)$ measurement shots has profound implications for the duty cycle of quantum processors. In current hardware, measurement and reset operations often take significantly longer than single-qubit and two-qubit gates. By reducing the number of required shots, the IQC framework enables higher throughput for classification tasks, making it a viable candidate for real-time quantum inference in hybrid systems. Furthermore, the structural robustness of the interference signal to noise suggests that this approach may remain reliable even on lower-fidelity devices where VQC optimization would fail to converge.

\subsection{Linearity and the Embedding Problem}

A fundamental limitation of the current IQC framework is its reliance on a linear decision boundary in Hilbert space. While the Hilbert space dimension $2^n$ is large, the separation of classes still depends heavily on the quality of the feature map $\Phi(\mathbf{x})$. If the classical data is mapped such that the class centroids are close together or heavily overlapping, the linear classifier will exhibit high bias. The dependence of shot complexity on the classification margin mirrors that of classical linear classifiers and reflects intrinsic ambiguity near the decision boundary rather than a limitation of the interference-based inference mechanism. This is a common challenge in quantum kernel methods as well. However, because our framework is static, one can theoretically employ more complex, high-dimensional classical feature engineering before state preparation to alleviate some of these linearity constraints.
...
The standard decomposition of such a unitary using the Plesch-Brukner algorithm \cite{plesch2011quantum} results in a circuit with $O(d)$ CNOT gates and $O(d)$ single-qubit rotations. While this is exponential in the number of qubits $n$, it is linear in the number of features $d$. For NISQ deployment, one often uses approximate state preparation techniques or hardware-efficient ansaetze that trade off exact representation for reduced depth. In our simulation, we assume an exact decomposition but note that the linear interference mechanism of the IQC remains valid regardless of the specific state preparation method used, as long as the relative phase information is preserved.

\section{Conclusion and Future Work}

This work presents an interference-based quantum classifier that operates as a fixed inference primitive. By focusing on the linear overlap $\mathrm{Re}\langle \phi | \psi \rangle$ rather than quadratic probabilities, we achieve $O(1)$ measurement efficiency and structural robustness to noise. Our approach decouples learning from quantum execution, using static class centroids computed classically. Experimental results demonstrate that this framework is competitive with variational models while being significantly more efficient in terms of measurement duty cycles and optimization overhead.
