...
Quantum machine learning (QML) offers transformative modeling potentials, but contemporary NISQ implementations face significant barriers, ranging from optimization instabilities in variational circuits to the heavy measurement demands of kernel-based methods \cite{benedetti2019parameterized,mcclean2018barren,schuld2019feature,havlicek2019supervised}. These constraints are particularly acute in histopathology, where the high dimensionality of 96\,$\times$\,96 image patches makes direct quantum feature mapping impractical on current hardware.
...
Variational quantum classifiers (VQCs) represent a prominent class of NISQ-era machine learning models \cite{benedetti2019parameterized,abbas2021power}. By employing parameterized quantum circuits (PQCs) as a flexible ansatz, VQCs optimize internal parameters to minimize a target classical cost function. While highly adaptable, these models often encounter training bottlenecks, such as the emergence of barren plateaus where gradients vanish exponentially in larger qubit registers \cite{mcclean2018barren}. Furthermore, the high measurement shot requirement for each optimization iteration can place a significant burden on quantum hardware.

\subsection{Quantum Kernel and Metric-Based Strategies}
Alternatively, quantum kernel methods avoid iterative parameter updates by mapping data into a high-dimensional Hilbert space to compute pairwise similarities \cite{schuld2019feature,havlicek2019supervised}. Often quantified using fidelity $F(\rho, \sigma)$, these similarity values are utilized as kernel entries in classical algorithms like Support Vector Machines (SVMs). However, these approaches remain measurement-intensive, typically scaling with $O(N)$ for a dataset of size $N$. Additionally, as a quadratic observable, fidelity may overlook phase-sensitive information crucial for specific decision boundaries.
...
\begin{enumerate}
    \item \textbf{Amplitude Encoding}: Encodes a normalized vector $\mathbf{x} \in \mathbb{R}^d$ into the amplitudes of a $2^n$-dimensional state. While requiring sophisticated state-preparation circuits, this provides optimal information density and ensures that the linear overlap in Hilbert space reflects the Euclidean dot product in the input space.
    ...
    \item \textbf{Basis-Level Encoding}: Maps bitstrings to computational basis states. This is computationally simpler but fails to leverage the superposition-driven interference advantage of the IQC.
\end{enumerate}
...
For our experiments, we utilize the PatchCamelyon (PCam) dataset, a cornerstone for benchmarking automated detection of lymph node metastases \cite{pcam_veeling2018}. PCam comprises 96\,$\times$\,96 RGB histopathology patches, where labels indicate the presence of metastatic tissue within the central 32\,$\times$\,32 pixel region. This binary classification task differentiates between tumor-infiltrated (malignant) and non-tumor (benign) tissue. The dataset's complexity stems from the intricate morphologies of lymphatic structures, where malignant cells disrupt normalized tissue architecture. Representative examples of these tissue types are shown in Fig.~\ref{fig:benign_malignant}.
...
A depolarizing channel $\mathcal{E}(\rho) = (1-p)\rho + p\frac{I}{2^{n+1}}$ transforms a state $\rho$ into a mixture with the maximally mixed state with probability $p$. For our interference circuit, this channel attenuates the overlap signal by a factor of $(1-p)^L$, where $L$ denotes circuit depth:
\begin{equation}
