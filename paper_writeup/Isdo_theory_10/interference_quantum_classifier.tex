...
Despite new quantum primitives, many NISQ classifiers are still constrained by unstable variational training or inference strategies that demand heavy measurement budgets \cite{benedetti2019parameterized,mcclean2018barren,schuld2019feature,havlicek2019supervised}. These constraints are particularly acute in histopathology, where the high dimensionality of 96\,$\times$\,96 image patches makes direct quantum feature mapping impractical on current hardware.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\linewidth]{{Interference-Based decision Geometery}.png}
...
The depolarizing channel is modeled as $\mathcal{E}(\rho) = (1-p)\rho + p\frac{I}{2^{n+1}}$, i.e., the state is replaced by the maximally mixed state with probability $p$. For our interference circuit, this channel attenuates the overlap signal by a factor of $(1-p)^L$, where $L$ denotes circuit depth:
\begin{equation}
...
Dephasing or phase-damping errors, $\mathcal{E}_{ph}(\rho) = (1-p)\rho + p Z\rho Z$, may arise during unitary execution or idle periods. Within the context of the Hadamard test, dephasing on the ancilla adds a stochastic phase $\theta$ to the interference term:
\begin{equation}
